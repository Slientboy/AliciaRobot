...
import AliciaRobot.modules.sql.feds_sql as sql
from AliciaRobot import (EVENT_LOGS, LOGGER, OWNER_ID, DRAGONS, TIGERS, WOLVES,
                          dispatcher)
...
from AliciaRobot.modules.helper_funcs.extraction import (extract_unt_fedban,
                                                          extract_user,
                                                          extract_user_fban)
    ...
    if args:
        if args[0] == 'json':
            ...
            with BytesIO(str.encode(backups)) as output:
                output.name = "alicia_fbanned_users.json"
                update.effective_message.reply_document(
                    document=output,
                    filename="alicia_fbanned_users.json",
            ...
            for users in getfban:
                ...
                backups += "\n"
            with BytesIO(str.encode(backups)) as output:
                output.name = "alicia_fbanned_users.csv"
                update.effective_message.reply_document(
                    document=output,
                    filename="alicia_fbanned_users.csv",
